...
This script is intended to explore the data. It is mainly focused on investigating missingness, finding outliers and checking data distributions.

```{r}
# Read data and filter to only rows with year 0 and year 2
dfHIV <- read.csv("./DataRaw/hiv_6624_final.csv") %>% 
  filter(years == 0 | years == 2)

# Count how many people have year 0 vs year 2
nrow(dfHIV %>% filter(years == 0))
nrow(dfHIV %>% filter(years == 2))

# Remove everyone who doesn't have year 2 outcomes
...
dfHIVWide <- dfHIVWide %>% mutate(logVLOAD_0 = log(VLOAD_0),
         ...
         logOfVLOAD_DIFF= log(VLOAD_2 - VLOAD_0))
...
# Diffs
hist(dfHIVWide$MENT_DIFF)

hist(dfHIVWide$PHYS_DIFF)

hist(dfHIVWide$LEU3N_DIFF)

hist(dfHIVWide$logVLOAD_DIFF)

hist(dfHIVWide$logOfVLOAD_DIFF)

```

```{r}
# Histograms for covariates and a look at the range of values for categorical variables
