...
1. Are the subject’s recordings of sampling times biased compared to the times recorded by the electronic monitoring cap?
...
The subject recordings were not biased relative to the cap times. The intercept and slope were not significantly different from the null hypothesis of 0 and 1 (table 2). This indicates that there was agreement between the booklet and cap times. 
...
Model coefficicents for the effect of time since wakeup on log(cortisol) are given in table 4. There was not a significant association between time since wakeup and log(cortisol) within the first 30 minutes of waking. For the first 30 minutes after waking, cortisol increased 0.64% per minute after wakeup (Wald test, 95% CI: -0.12%, 1.41%, p = 0.10). The change at 30 minutes was significant (p = 0.03). After 30 minutes, cortisol decreased 0.22% per minute after wakeup (Wald test, 95% CI: 0.19%, 0.26%, p < 0.001). The predicted log(cortisol) level for an average subject is given in figure 2. 
```{r, echo=FALSE, warning=FALSE, message=FALSE, results='hide'}
...
# calculate percent changes for betas
(exp(confint(cortisolModel))) * 100
(1 - exp(confint(cortisolModel))) * 100

betaCortisol <- summary(cortisolModel)$coefficients[2] + summary(cortisolModel)$coefficients[3]
100 - exp(betaCortisol) * 100
...
pnorm(betaCortisol / sqrt(sdCortisol)) 
```
```{r, echo=FALSE, warning=FALSE, message=FALSE}
...
Code located at https://github.com/isabellemeredith/BIOS6624/tree/main/Project0. Data is private and available upon request. 
